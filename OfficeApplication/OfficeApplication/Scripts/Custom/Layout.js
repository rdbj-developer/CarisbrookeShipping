...
function PerformClientActionsBasedOnCommonServerCallResponse(data, strAction){
    switch (strAction) {
        // JSL 02/18/2023
        case str_API_INSERTORUPDATEFSTO:
            PerformClientInsertOrUpdateFSTO(data);
            break;
        // JSL 02/18/2023
        // JSL 02/17/2023
        case str_API_GETFSTODETAILSBYID:
            PerformClientGetFSTODetailsById(data);
            break;
        // JSL 02/17/2023
        // JSL 02/17/2023
        case str_API_DELETEFSTO:
            PerformClientDeleteFSTO(data);
            break;
        // JSL 02/17/2023
        // JSL 02/17/2023
        case str_API_DELETEFSTOFILE:
            PerformClientDeleteFSTOFile(data);
            break;
        // JSL 02/17/2023
        // JSL 07/23/2022
...
// JSL 02/18/2023
function PerformClientInsertOrUpdateFSTO(data) {
    if (data.Status == str_Status_Success) {
        var blnIsActionForUpdate = JSON.parse(data.IsActionForUpdate);
        $.notify("FSTO saved successfully!", "success");

        InitShipFSTOAudits($("#ddlGIReportShipName option:selected").val());    // JSL 02/21/2023 added actual ship selected
        $('#fstoDetailsGrid').empty();
        GetFSTOItemsForEdit();
        closeFSTOModal();

        if (blnIsActionForUpdate)
            $("#fstoGrid tbody tr:nth-child(" + rowAuditIndex + ")").trigger('click');
        else
...
// JSL 02/17/2023
function PerformClientGetFSTODetailsById(data) {
    if (data.Status == str_Status_Success) {
        var objFSTO;
        if (!IsNullEmptyOrUndefined(data.FSTOData)) {
            objFSTO = JSON.parse(data.FSTOData);
        }

        SetValueAndOpenModalForEdit(objFSTO);
    }
}
// End JSL 02/17/2023

// JSL 02/17/2023
function PerformClientDeleteFSTO(data) {
    if (data.Status == str_Status_Success) {
        var element;
        var element = $('td.fstoUniqueFormID').filter(function () {
            return this.textContent.trim() === data.UniqueFormID
        });
...
// JSL 02/17/2023
function PerformClientDeleteFSTOFile(data) {
    if (data.Status == str_Status_Success) {
        var element;
        element = document.querySelector('[data-id="dvFSTO_' + data.FileUniqueID + '"]');
        $(element).remove();    
        $.notify("Performed Action done Successfully!", "success");
    }
}
...
// JSL 02/17/2023
function BindDeletePopConfirm(eleSelector, strPlacement = "right") {
    $(eleSelector).popConfirm({
        title: "Delete!",

...
var menuDeficiency;
var itemNumber = "";
var blnUpdateButton = false; //RDBJ 10/06/2021
...
$(document).ready(function () {
    ...
    $(".NavButton").click(function () {
        $("button").removeClass("active"); //RDBJ 10/19/2021
        $(this).addClass("active"); //RDBJ 10/19/2021
    ...
    $("#Child").change(function () {
        ...
        //10/07/2021
        if ($("#GIRForm").valid()) {
            GIRAutoSave(true);  // RDBJ 04/18/2022 pass true
        }
    });
    ...
    $("#DeficienciesSection").click(function () {
        ...
        //if ($("#appMode").val().toUpperCase() == "ONLINE") {
        //    FormId = $("#OfficeGIRFormID").val();
        //} else {
        //    FormId = $("#GIRFormID").val();
        //}
        $.ajax({
            type: "GET",
            url: url + "?id=" + FormId,
    ...
    $("#btnUpdateShipsData").click(function () {
        if (!blnUpdateButton) {
            ...
            $("#btnUpdateShipsDataCancel").css("display", "block"); //RDBJ 10/07/2021
            AllowEditShipGeneralDescription();
            blnUpdateButton = true;
        }
        else {
            $("#btnUpdateShipsData").text("Edit Ships Data");
            $("#btnUpdateShipsDataCancel").css("display", "none"); //RDBJ 10/07/2021
    ...
    if (isInspector == 'True') {
        menuDeficiency = $("#menu").kendoContextMenu({
            ...
            select: function (e) {
                ...
                if ($("#UniqueFormID").val() != "00000000-0000-0000-0000-000000000000" && $("#UniqueFormID").val() != "") { //RDBJ 11/02/2021 added UniqueFormID //RDBJ 09/18/2021 changed condition if ($("#GIRFormID").val() != "0") {
                    ...
                    var Modal = {
                        GIRFormID: 0, //$("#GIRFormID").val(), // RDBJ 12/08/2021 set 0
                        OfficeGIRFormID: $("#OfficeGIRFormID").val(),
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            $.notify("Deficiency Added Successfully", "success");  // RDBJ 02/20/2022
...
function ValidateGIRForm() {
    ...
    $('.txtDateRaised').datepicker({
        format: 'dd/mm/yyyy', //RDBJ 11/28/2021
        autoclose: true
    });
    ...
    GIRFormValidator = $("#GIRForm").validate({
        ...
        messages: {
            ...
            txtDeficiency: "Please Enter Deficiency", //RDBJ 10/30/2021
            dateRaised: "Please Enter Date", //RDBJ 10/30/2021
        },
...
function GIRAutoSave(
    ...
    var url = RootUrl + 'Forms/GIRAutoSave';
    $("#lblAutoSave").show();
    ...
    $.ajax({
        ...
        success: function (res) {
            console.log(res.OffGIRFormId);
            console.log(res.GIRFormId);
            //$("#GIRFormID").val(res.GIRFormId);
            $("#OfficeGIRFormID").val(res.OffGIRFormId);
...
function printDiv() {
    ...
    setTimeout(function () {
        ...
        // RDBJ 01/07/2022 this is only for DeficienciesSections
        $(".txtAreaDeficiencies").each(function () {
            $(this).css("width", "100%");
            this.addEventListener('focus', function () {
                autosize(this);
...
function GetGenDescData(id) {
    var url = RootUrl + 'Forms/ShipDetails';
    $.ajax({
        type: 'POST',
        url: url,
        data: { shipCode: id }, //RDBJ 09/17/2021 set shipCode from Shipvalue
        dataType: 'json',
        success: function (data) {
            SetGenDescData(data);
        },
...
function SetGenDescData(data) {
    if (data != null) { //RDBJ 09/17/2021 remove condition && data.length > 0

        //RDBJ 09/17/2021 Removed array getting value from all columns data[0] to set with data
        // RDBJ 01/11/2022 commented below 3 lines
        /*
        ...
        */
        ...
        //End RDBJ 09/17/2021 Removed array getting value from data[0] to set with data
    }
    else {
        SetGenDescDataEmpty();
    }
...
function GIRShipGeneralDescriptionSave() {
    var url = RootUrl + 'Forms/GIRShipGeneralDescriptionSave';
    $("#lblAutoSave").show();
    var form = $("#GIRForm");
    $.ajax({
        url: url,
...
function RemoveRequiredStarsIfValues() {
    ...
    if ($("#Date").val() == "")
        $("#lblErrorDate").css("display", "inline");
    else

...
$(document).ready(function () {
    ...
    // RDBJ 03/01/2022 commented to avoid load page call, not needed
    /*
...
function LoadAuditShips() {
    var url = RootUrl + "Deficiencies/GetAuditShipsDeficincyGrid";
    $.ajax({
        type: 'GET',
        dataType: 'json',
        async: false,
...
function InitShipAudits(shipCode) {
    ...
    var grid = $('#Grid').kendoGrid({
        ...
        columns: [
            ...
            {
                field: "Subject",
                title: "Audit", // RDBJ 01/31/2022 set Audit from Subject
                width: 70,
            ...
            {
                template: '<input type="checkbox" #= Extra ? \'checked="checked"\' : "" # class="chkbx" />',
                title: "Extra?",
            ...
            // JSL 05/20/2022 re-order columns
            // RDBJ 01/19/2022 re-order columns
            {
                field: "NCN",
                title: "No. Of NCN's",
                width: 50,
            ...
            {
                template: '<input type="checkbox" #= Closed ? \'checked="checked"\' : "" # class="chkbx" disabled />',  // JSL 05/19/2022 added disabled
                title: "Closed?",
                width: 80,
                attributes: { class: "mainContext" },
            },
            ...
            // End RDBJ 01/19/2022 re-order columns
            // RDBJ 01/31/2022
            {
            ...
            // End RDBJ 01/31/2022
            // End JSL 05/20/2022 re-order columns
        ]
    });
    setTimeout(function () {
        var gridDataDetail = $('#Grid').data("kendoGrid");
...
function LoadAuditDetail() {
    $("#Grid tbody").on("click", "tr", function (e) {
        ...
        var uniqueFormID = e.currentTarget.children[0].textContent
        var Subject = e.currentTarget.children[2].textContent   // RDBJ 02/11/2022 set index 2
        if (AuditID != "" && Subject != "") {
            var url = RootUrl + "Deficiencies/GetAuditDetails/" + uniqueFormID;
            $.ajax({
                type: 'GET',
                dataType: 'json',
                async: false,
                ...
                success: function (Data) {
                    ...
                    var grid = $('#DetailsGrid').kendoGrid({
                        ...
                        change: function () {
                            ...
                            var uniqueFormID = row[0].cells[1].textContent;
                            var notesUniqueID = row[0].cells[0].textContent;    // RDBJ 01/31/2022 set Index

                            if (FormID && FormID != "") {
                                var _url = RootUrl + "Deficiencies/InternalAuditDetails?id=" + notesUniqueID;
                        ...
                        columns: [
                            ...
                            },
                            {
                                field: "IsResolved",
                                title: "Resolved?",
                                template: '<input  type="checkbox" #= IsResolved ? \'checked="checked"\' : "" # class="chkbx" disabled />', // JSL 05/20/2022 removed onclick="updateStatus(this)" and Set disabled
                                width: "120px",
                                // hidden: true,    // JSL 05/20/2022 commented this line
                            },
                            {
                                field: "DueDate",
                                title: "DueDate",
                            ...
                            {
                                field: 'AuditNotesAttachment',
                                title: 'Files',
                                template: "#=generateTemplateAuditNotesAttachment(AuditNotesAttachment)#",
                                width: "20%",
                                hidden: true,   // RDBJ 01/31/2022
                            },
                        ]
                    });
                    setTimeout(function () {
...
function generateTemplateAuditNotesAttachment(AuditNoteFileList) {
    var template = "";
    for (var i = 0; i < AuditNoteFileList.length; i++) {
        template = template + '<div style="line-height: unset;" class="chip pink lighten-2 white-text waves-effect waves-effect file">' +
            '<a onclick="Download(\'' + AuditNoteFileList[i].NotesFileUniqueID + '\',this)">' + AuditNoteFileList[i].FileName + '</a>' +
            '</div>';
    }
    return template;
}
...
function generateTemplate(ReportList) {
    ...
    for (var i = 0; i < ReportList.length; i++) {
        template = template + '<div style="line-height: unset;" class="chip pink lighten-2 white-text waves-effect waves-effect file">' +
            '<a onclick="DownloadAuditCommentFile(' + ReportList[i].CommentFileID + ',this)">' + ReportList[i].FileName + '</a>' +
            '</div>';
    }
    return template;
}
function DownloadAuditCommentFile(ctr, data) {
    var _name = $(data).text();
    var url = RootUrl + "Deficiencies/DownloadAuditCommentFile?CommentFileID=" + ctr + "&name=" + _name;;
    $.ajax({
        url: url,
        contentType: 'application/json; charset=utf-8',
        datatype: 'json',

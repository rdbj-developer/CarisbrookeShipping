...
{
    public class CloudLocalGIRSynch
    {
        string APIUrl = System.Configuration.ConfigurationManager.AppSettings["APIUrl"];
        public APIResponse sendGIRLocalToRemote(GeneralInspectionReport Modal)
        {
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    // HTTP POST
                    HttpResponseMessage resMsg = client.PostAsync("api/CloudGIR/sendGIRLocalToRemote", content).Result;
            ...
            {
                LogHelper.writelog("sendGIRLocalToRemore GIR Form Error : " + ex.Message.ToString());
                LogHelper.writelog("sendGIRLocalToRemore GIR Form " + Modal.UniqueFormID.ToString() + " Error : " + ex.InnerException.ToString()); // RDBJ 01/21/2022
            }
            return response;
        }
    }
